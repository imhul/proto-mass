...
const Objects = memo(() => {
    ...
    useEffect(() => {
        if (!objectList.length && !isObjectsCreation && isMapLoaded && isFirstResize) {
            dispatch({ type: 'OBJECTS_CREATION_START' });
            indicateStart();
        }
    }, [dispatch, isObjectsCreation, objectList.length, isMapLoaded, indicateStart, isFirstResize]);
    ...
    useEffect(() => {
        const objects = getMap.flat().map((tileTypeId, index) => {
            ...
            // logic
            const copies = objectList.filter(
                item =>
        ...
        if (objects && objects.length > objectList.length && isObjectsCreation) {
            const filled = objects.filter(item => item !== null && item.typeId !== '');
            dispatch({ type: 'OBJECTS_CREATED', payload: filled });
            indicateDone();
        }
    }, [dispatch, idLength, isObjectsCreation, objectList, indicateDone]);
